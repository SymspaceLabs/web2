...
export class MailchimpService {
  ...
  // Corrected mailchimp.service.ts
  async sendEmailWithTemplate(toEmail: string, subject: string, otp: string, templateId: string) {
      try {
          const templateName = templateId;

          const response = await this.mailchimpClient.messages.sendTemplate({
              template_name: templateName,
              // Removed template_content as it's not needed if you use merge tags.
              template_content: [], 
              message: {
                  to: [{ email: toEmail, type: 'to' }],
                  from_email: 'contact@symspacelabs.com',
                  from_name: 'Symspace Labs',
                  subject,
                  global_merge_vars: [
                      {
                          name: 'otp', // Matches the {{otp}} tag from your HTML.
                          content: otp,
                      },
                  ],
              },
