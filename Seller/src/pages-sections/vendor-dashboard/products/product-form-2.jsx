...
const ProductForm2 = forwardRef((props, ref) => {
    ...
    // =============================================================================
    // ⭐ 1. INITIALIZATION HELPERS (Maps and Lookups) ⭐
    // =============================================================================
    ...
    const initialVariantIdMap = useMemo(() => {
        ...
        const map = variantsArray.reduce((acc, variant) => {
            // ⭐ CRITICAL FIX: Handle null color from API
            const colorName = variant.color?.name || variant.color || DEFAULT_COLOR_KEY; // 👈 CRITICAL EDIT
            const sizeName = variant.size?.size || variant.size; 
            
            // Key must exactly match the key generated by the ProductVariantsTable
            const key = `${colorName}-${sizeName || 'null'}`; // 👈 CRITICAL EDIT: Include '|| null' for size in the key
            
            acc[key] = {
                // ⭐ Ensure ID is available in the initial state object
                id: variant.id, 
    ...
    const initialVariantsMap = useMemo(() => {
        ...
        const map = variantsArray.reduce((acc, variant) => {
            // ⭐ CRITICAL FIX: Access the name/size from the nested objects
    ...
    // ----------------------------------------------------------------------------------
    // ⭐ HANDLER: Logic to receive variants from table and re-inject IDs (LOOP PREVENTION FIX) ⭐
    // ----------------------------------------------------------------------------------
